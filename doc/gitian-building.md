...
*Setup instructions for a gitian build of MegaByteCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MegaByteCoin
...
- [Building MegaByteCoin](#building-megabytecoin)
...
Clone the git repositories for MegaByteCoin and gitian and then checkout the MegaByteCoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/megabytecoincoin/megabytecoin-core.git
...
Follow the instructions in [doc/release-process.md](release-process.md) in the megabytecoin-core repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building MegaByteCoin
-------------

To build MegaByteCoin (for Linux, OSX, and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the megabytecoin-core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/megabytecoin-core/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/megabytecoincoin/megabytecoin-core
...
./bin/gbuild --commit megabytecoin=${COMMIT} --url megabytecoin=${URL} ../megabytecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit megabytecoin=${COMMIT} --url megabytecoin=${URL} ../megabytecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit megabytecoin=${COMMIT} --url megabytecoin=${URL} ../megabytecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/megabytecoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/megabytecoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/megabytecoin-build.assert
```
...
[megabytecoincoin/gitian.sigs](https://github.com/megabytecoincoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
